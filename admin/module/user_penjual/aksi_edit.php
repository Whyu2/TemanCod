...
$status = $_POST ['status'];
    ...
    if ($queryedit){
        echo "<script> alert ('Status user berhasil diubah'); window.location ='$admin_url'+ 'adminweb.php?module=user_penjual';</script>";
    }else {
        echo "<script> alert ('Status user gagal diubah'); window.location ='$admin_url'+ 'adminweb.php?module=user_penjual&id_user='+'$id_user';</script>";
    }
    }
}
    ?>